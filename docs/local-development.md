...
| Make | Optional | Provides shortcut commands such as `make verify`. |
...
If `python` is not found but `python3` is, substitute `python3` in the commands
below. On Windows PowerShell, activate the virtual environment with
...
or `make` commands for this repository.
...
Why port 5001? The checked-in Flask CLI settings use port 5001. The frontend
template and Vite proxy still show port 5000 for compatibility, so explicitly
setting the frontend variable to port 5001 avoids sending browser requests to
the wrong server.
...
Without `make`, use the equivalent commands described in the root README.
...
local SQLite data, stop Flask, delete the ignored `app.db` file, and run the
migration command again. This deletes local data.
